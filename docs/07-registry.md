...
By default `src/index.ts` creates `export const registry = new ModuleRegistry();` then calls `registry.registerAll(basicModules)` to preload built-ins.
...
`registry.list()` returns the current module info, which `buildAIPlanPrompt()` and `buildQplanSuperPrompt()` feed directly into LLM prompts.
...
1. `runQplan(script)` parses to AST, then the executor runs steps.
...
- **Removing modules**: not supported. Instead create a new `ModuleRegistry`, register only what you need, and pass it to a custom `runQplan` entry point.
...
With this guide you can see how ModuleRegistry underpins QPlan’s module ecosystem and how it powers both execution and LLM integrations.
