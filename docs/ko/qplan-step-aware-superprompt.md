...
- 하나의 Step에 전 과정(Action)을 몰아넣지 말고, **작업 목적별로 Step을 분리**한다. (예: 데이터 수집/전처리/요약/보고 등)
- 상위 Step은 오케스트레이션이나 별도 작업이 있을 때만 만들고, **아무 로직 없이 하위 Step만 감싸는 래퍼 Step은 금지**한다.
- Step 안에는 서로 강하게 연관된 Action만 2~3개 정도 묶어 넣고, 역할이 다르면 새로운 Step으로 분리한다.
- 필요하다면 `step id="prepare"` / `step id="analyze"` / `step id="report"` 처럼 **각 단계가 수행하는 동작을 이름과 desc로 명확히 드러내도록** 한다.
