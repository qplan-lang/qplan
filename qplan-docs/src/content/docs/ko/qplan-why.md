...
언어 중립적·안전 기반 AI Planning Language이다.  
...
# 4. 비동기(Future), 병렬(Parallel) 제어를 Language로 보장
...
Step 결과나 JSON 객체를 저장해 두면 `sum.detail.total` 처럼 점(dot) 표기로 바로 참조할 수 있어, 상태 전달이 더욱 단순해진다.
